...
impl StringToEvmEncoder {
    ...
    fn init_abi_cache(&mut self) {
        ...
        self.abi_cache.insert(
            "bridgeIn(bytes32,uint256,bytes)".to_string(),
            vec![0xb1, 0x2c, 0x4e, 0x8f],
        ); // bridgeIn (custom)
        self.abi_cache.insert(
            "bridgeOut(bytes32,uint256)".to_string(),
            vec![0xc2, 0x3d, 0x5f, 0xa0],
        ); // bridgeOut (custom)
    }
    ...
    fn encode_contract_call(
        ...
        let selector = self.abi_cache.get(method).cloned().unwrap_or_else(|| {
    ...
    fn get_token_contract(&self, token: &str) -> Result<[u8; 20], EncodingError> {
        ...
        match token.to_uppercase().as_str() {
            ...
            "FAT" | "DCFAT" => {
                ...
                Ok([
                    0x0b, 0x44, 0x54, 0x7b, 0xe0, 0xa0, 0xdf, 0x5d, 0xcd, 0x53, 0x27, 0xde, 0x8e,
                    0xa7, 0x36, 0x80, 0x51, 0x7c, 0x5a, 0x54,
                ])
            }
    ...
    fn get_bridge_contract(&self, chain: &str) -> Result<[u8; 20], EncodingError> {
        match chain.to_lowercase().as_str() {
            "ethereum" | "eth" => Ok([
                0x0b, 0x44, 0x54, 0x7b, 0xe0, 0xa0, 0xdf, 0x5d, 0xcd, 0x53, 0x27, 0xde, 0x8e, 0xa7,
                0x36, 0x80, 0x51, 0x7c, 0x5a, 0x54,
            ]),
            "xdc" => Ok([
                0x20, 0xb5, 0x9e, 0x6c, 0x5d, 0xeb, 0x7d, 0x7c, 0xed, 0x2c, 0xa8, 0x23, 0xc6, 0xca,
                0x81, 0xdd, 0x3f, 0x7e, 0x9a, 0x3a,
            ]),
            "rope" | "datachain" => {
    ...
    fn parse_address(&self, address: &str) -> Result<[u8; 20], EncodingError> {
        ...
        let bytes =
            hex::decode(hex_str).map_err(|_| EncodingError::InvalidAddress(address.to_string()))?;
...
impl EvmToStringDecoder {
    ...
    fn decode_contract_call(
        ...
        if selector == [0xa9, 0x05, 0x9c, 0xbb] {
            if tx.data.len() >= 68 {
                ...
                return Ok(DecodedStringOperation {
                    ...
                    rope_concept: RopeConcept::TokenTransfer { token_id, amount },
                    metadata: [("recipient".to_string(), hex::encode(recipient))]
                        .into_iter()
                        .collect(),
                });
            }
        ...
        if selector == [0xb1, 0x2c, 0x4e, 0x8f] {
            if tx.data.len() >= 68 {
                ...
                return Ok(DecodedStringOperation {
                    ...
                    metadata: [("amount".to_string(), tx.value.to_string())]
                        .into_iter()
                        .collect(),
                });
            }
        ...
        Ok(DecodedStringOperation {
            ...
            rope_concept: RopeConcept::String {
                id: *blake3::hash(&tx.data).as_bytes(),
            },
            metadata: [
...
impl EvmInvocationBridge {
    ...
    pub async fn process_evm_event(
        ...
        match event {
            ...
            EvmBridgeEvent::Withdrawal {
                ...
                if !self.verify_withdrawal_proof(&proof) {
                    return Err(BridgeError::VerificationFailed(
                        "Invalid withdrawal proof".to_string(),
                    ));
                }
            ...
            EvmBridgeEvent::ContractCallback {
                invocation_id,
                result,
            } => {
...
mod tests {
    ...
    fn test_evm_transaction_encoding() {
        ...
        let result = encoder.encode_string_to_evm(&[0u8; 32], &operation, &[0x42; 32], 0);
    ...
    fn test_state_proof_generation() {
        ...
        let strings = [[1u8; 32], [2u8; 32], [3u8; 32], [4u8; 32]];
