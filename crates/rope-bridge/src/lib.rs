    ...
    impl std::fmt::Display for BridgeError {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            match self {
                ...
                BridgeError::InvalidProof(s) => write!(f, "Invalid proof: {}", s),
                BridgeError::InvalidPayload(s) => write!(f, "Invalid payload: {}", s),
...
pub mod ethereum {
    ...
    /// Ethereum bridge implementation with JSON-RPC support
    ...
    impl EthereumBridge {
        pub fn new(config: EthereumConfig) -> Self {
            Self {
                ...
                latest_block: std::sync::atomic::AtomicU64::new(0),
                chain_id: std::sync::atomic::AtomicU64::new(0),
            }
        }
        ...
        /// Verify a Merkle proof against state root
    ...
    impl Bridge for EthereumBridge {
        ...
        async fn verify_proof(&self, proof: &[u8]) -> Result<bool, BridgeError> {
            // Parse proof structure: root (32) + key_len (4) + key + value_len (4) + value + proof_nodes
            if proof.len() < 40 {
    ...
    impl SemanticTranslator {
        ...
        fn register_default_mappings(&mut self) {
            ...
            let string_to_tx = SemanticMapping {
                ...
                rules: vec![TransformationRule {
                    ...
                    field_mapping: [("string_id".to_string(), "hash".to_string())]
                        .into_iter()
                        .collect(),
                    value_transform: Some(ValueTransform::Hash {
            ...
            let token_to_erc20 = SemanticMapping {
                ...
                external_concept: ExternalConcept::Erc20Transfer {
                    contract: [0u8; 20],
                    amount: 0,
                },
                rules: vec![TransformationRule {
                    name: "amount_scaling".to_string(),
                    field_mapping: [("amount".to_string(), "amount".to_string())]
                        .into_iter()
                        .collect(),
...
pub mod encapsulation {
    ...
    impl EncapsulationEngine {
        ...
        ) -> Result<EncapsulatedTransaction, String> {
            // Generate commitment
...
mod tests {
    ...
    fn test_bridge_transaction_creation() {
        let tx = BridgeTransaction {
            id: [1u8; 32],
            source_string_id: [2u8; 32],
            target_protocol: ProtocolType::Blockchain(BlockchainType::Ethereum),
