...
impl FinalityState {
    ...
    pub fn is_rejected(&self) -> bool {
        matches!(self, FinalityState::Rejected { .. } | FinalityState::Expired { .. })
    }
...
impl FinalityEngine {
    ...
    fn check_and_update_finality(&self, info: &mut StringFinalityInfo, anchor_id: &[u8; 32]) {
        ...
        let has_anchor_confirmations = info.anchor_confirmations >= self.config.min_anchor_confirmations;
    ...
    fn calculate_confidence(
        ...
        let anchor_score = (anchor_confirmations as u8 * 40 / self.config.min_anchor_confirmations as u8)
            .min(40);
        score += anchor_score;
        
        // Testimony count (up to 40 points)
        let testimony_score = (testimony_count as u8 * 40 / self.config.min_testimonies as u8)
            .min(40);
        ...
        for info in strings.values_mut() {
            if info.state.is_pending() || matches!(info.state, FinalityState::Tentative { .. }) {
                if now - info.created_at > timeout {
                    info.state = FinalityState::Expired {
                        expired_at: now,
                    };
                }
            }
