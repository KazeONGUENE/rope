...
impl TestimonyCollection {
    ...
    pub fn count_type(&self, attestation_type: AttestationType) -> usize {
        self.type_counts.get(&attestation_type.as_u8()).copied().unwrap_or(0)
    }
    ...
    pub fn unique_validators(&self) -> Vec<NodeId> {
        let mut validators: Vec<NodeId> = self.testimonies
            .iter()
...
impl TestimonyCollector {
    ...
    pub fn register_validator(&self, validator: NodeId) {
        ...
        if !validators.iter().any(|v| v.as_bytes() == validator.as_bytes()) {
    ...
    fn validate_testimony(&self, testimony: &Testimony) -> Result<(), TestimonyError> {
        ...
        if !validators.iter().any(|v| v.as_bytes() == testimony.validator_id.as_bytes()) {
...
impl FinalityProgress {
    ...
    pub fn percentage(&self) -> f64 {
        ...
        (self.current_testimonies as f64 / self.required_testimonies as f64 * 100.0)
            .min(100.0)
    }
}
