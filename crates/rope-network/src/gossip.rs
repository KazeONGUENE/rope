...
impl GossipHistory {
    ...
    pub fn latest_round(&self) -> u64 {
        self.events.back().map(|e| e.round).unwrap_or(0)
    }
}
...
impl GossipProtocol {
    ...
    pub fn create_have_message(&self) -> Option<GossipMessage> {
        ...
        let string_ids: Vec<StringId> = pending
            .iter()
    ...
    pub fn handle_message(&self, msg: GossipMessage) -> Option<GossipMessage> {
        ...
        match msg.message_type {
            ...
            GossipMessageType::Want(ids) => {
                ...
                let pending = self.pending_strings.read();
                let data: Vec<StringData> = pending
                    .iter()
    ...
    pub fn record_event(&self, receiver_id: [u8; 32], round: u64, string_ids: Vec<StringId>) {
        let timestamp = chrono::Utc::now().timestamp();
