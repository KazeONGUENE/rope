...
use rope_core::string::{RopeString, PublicKey, HybridSignature};
use rope_core::types::{StringId, NodeId, MutabilityClass};
...
impl StringProducer {
    ...
    pub async fn run(&mut self, mut shutdown_rx: mpsc::Receiver<()>) {
        ...
        info!(
            ...
            self.config.string_interval_ms,
            self.config.min_testimonies
        );
    ...
    fn produce_anchor(&self) -> anyhow::Result<StringId> {
        ...
        let parent_id = self.last_anchor_id.read()
