...
impl Federation {
    ...
    pub fn register_validator(&self, node_id: [u8; 32], stake: u128, commission: u16) -> Result<(), FederationError> {
    ...
    pub fn add_stake(&self, node_id: &[u8; 32], amount: u128) -> Result<u128, FederationError> {
        let mut validators = self.validators.write();
        let validator = validators.get_mut(node_id)
    ...
    pub fn withdraw_stake(&self, node_id: &[u8; 32], amount: u128) -> Result<u128, FederationError> {
        let mut validators = self.validators.write();
        let validator = validators.get_mut(node_id)
            .ok_or(FederationError::ValidatorNotFound)?;
    ...
    pub fn record_missed_attestation(&self, node_id: &[u8; 32]) -> Result<bool, FederationError> {
        ...
        if validator.missed_attestations >= 100 {
            drop(validators);
            self.slash(node_id, SlashingReason::Downtime { 
                missed_count: 100 
            })?;
            return Ok(true);
        }
    ...
    /// Slash a validator
    pub fn slash(&self, node_id: &[u8; 32], reason: SlashingReason) -> Result<u128, FederationError> {
        let mut validators = self.validators.write();
        let validator = validators.get_mut(node_id)
            .ok_or(FederationError::ValidatorNotFound)?;
    ...
    pub fn next_epoch(&self) -> Epoch {
        ...
        let mut sorted: Vec<_> = validators.values()
            .filter(|v| v.stake >= self.min_stake)
            .collect();
        sorted.sort_by(|a, b| b.stake.cmp(&a.stake));
        
        let active_validators: Vec<_> = sorted.iter()
            .take(self.max_validators)
            .map(|v| v.node_id)
            .collect();
        
        let total_stake: u128 = sorted.iter()
            .take(self.max_validators)
            .map(|v| v.stake)
            .sum();
...
mod tests {
    ...
    fn test_insufficient_stake() {
        ...
        assert!(matches!(result, Err(FederationError::InsufficientStake { .. })));
    }
    ...
    fn test_slashing() {
        ...
        let slashed = federation.slash(&[1u8; 32], SlashingReason::DoubleSign).unwrap();
    ...
    fn test_epoch_transition() {
        ...
        // Register validators
        for i in 1..=5 {
            federation.register_validator([i as u8; 32], i as u128 * 1000, 500).unwrap();
        }
