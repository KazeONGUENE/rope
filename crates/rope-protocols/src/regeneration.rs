...
impl DamageType {
    ...
    pub fn severity(&self) -> u8 {
        match self {
            ...
            DamageType::SevereCorruption {
                recovery_chance_percent,
            } => 100 - recovery_chance_percent,
...
pub enum RepairResult {
    ...
    /// Repair failed
    Failed { string_id: [u8; 32], reason: String },

    /// String is unrecoverable
    Unrecoverable { string_id: [u8; 32], reason: String },
}
...
impl RegenerationCoordinator {
    ...
    pub fn try_complete_repair(&self, request_id: &[u8; 32]) -> Option<RepairResult> {
        ...
        let (consensus_hash, count) = hash_counts.iter().max_by_key(|(_, count)| *count).unwrap();

        // Require majority for multi-source strategies
        if matches!(
            request.strategy,
            RepairStrategy::MultiSourceVerify | RepairStrategy::MultiSourceReconstruct
        ) {
        ...
        let repair_data = valid_responses
            .iter()
...
impl DamageDetector {
    ...
    pub fn register_string_pair(
        ...
        let primary_integrity =
            StringIntegrity::new(primary_id, primary_content, 4096).with_complement(complement_id);
    ...
    pub fn quick_check(&self, string_id: &[u8; 32], content: &[u8]) -> Option<DamageReport> {
        ...
        let corruption_percent =
            (damaged_segments.len() as f64 / integrity.segments.len() as f64) * 100.0;
    ...
    pub fn compare_with_complement(
        ...
        for (i, (a, b)) in primary_content
            .iter()
            .zip(complement_content.iter())
            .enumerate()
        {
        ...
        Some(DamageReport {
            ...
            corruption_percent: (different_positions.len() as f64 / primary_content.len() as f64)
                * 100.0,
            detected_at: chrono::Utc::now().timestamp(),
            method: DetectionMethod::ComplementComparison,
        })
...
impl ReedSolomonCodec {
    ...
    pub fn decode(&self, mut rs_data: ReedSolomonData) -> Result<Vec<u8>, String> {
        ...
        self.encoder
            .reconstruct(&mut rs_data.shards)
...
impl NetworkRepairCoordinator {
    ...
    pub fn try_complete(&self, request_id: &[u8; 32]) -> Option<Vec<u8>> {
        ...
        let (best_hash, best_data) = hash_votes.into_iter().max_by_key(|(_, v)| v.len())?;
...
mod damage_detection_tests {
    ...
    fn test_reed_solomon_encode_decode() {
        ...
        let original =
            b"Test data for Reed-Solomon encoding and recovery! This needs to be longer.";
    ...
    fn test_erasure_coding_manager() {
        ...
        // High redundancy
        let encoded_hr = manager
            .encode_high_redundancy(original)
            .expect("HR encode failed");
