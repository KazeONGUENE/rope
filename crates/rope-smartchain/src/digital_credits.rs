...
pub enum OperationType {
    ...
    Approve { spender: [u8; 32], amount: Balance },
    ...
    /// Unfreeze tokens
    Unfreeze { account: [u8; 32], amount: Balance },

    /// Update token metadata
    UpdateMetadata { metadata: TokenMetadata },
...
impl CreditsLedger {
    ...
    pub fn mint(
        ...
        let operation_id =
            *blake3::hash(&[to.as_slice(), token_id.as_slice(), &amount.to_le_bytes()].concat())
                .as_bytes();
    ...
    fn credit_account(
        ...
        let acc = accounts
            .entry(*account)
            .or_insert_with(|| Account::new(*account));
...
mod tests {
    ...
    fn test_mint_tokens() {
        ...
        let result = ledger
            .mint(&token_id, &recipient, 500_000, &creator, false)
            .unwrap();
