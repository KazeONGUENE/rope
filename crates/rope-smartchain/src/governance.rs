...
impl MintingGovernance {
    ...
    pub fn select_governors(
        &self,
        entropy: &[u8; 32],
    ) -> Result<GovernorSelection, GovernanceError> {
        ...
        let active_foundation: Vec<_> = foundation.iter().filter(|m| m.is_active).collect();
        ...
        for i in 0..self.config.required_random_governors {
            selection_state =
                *blake3::hash(&[&selection_state[..], &i.to_le_bytes()].concat()).as_bytes();
            ...
            while selected_governors.contains(&candidate) && attempts < 100 {
                ...
                let new_index = u64::from_le_bytes(selection_state[0..8].try_into().unwrap())
                    as usize
                    % validator_list.len();
        ...
        let foundation_wallets: Vec<_> = active_foundation
            .iter()
    ...
    pub fn submit_ai_approval(
        ...
        if proposal
            .ai_approvals
            .iter()
            .any(|a| a.agent_id == approval.agent_id)
        {
        ...
        let approved_count = proposal.ai_approvals.iter().filter(|a| a.approved).count();
    ...
    pub fn submit_governor_approval(
        ...
        if !proposal
            .governor_selection
            .random_governors
            .contains(&approval.governor_wallet)
        {
        ...
        if proposal
            .governor_approvals
            .iter()
            .any(|a| a.governor_wallet == approval.governor_wallet)
        {
            return Err(GovernanceError::AlreadyVoted);
        }
    ...
    pub fn submit_foundation_approval(
        ...
        if !proposal
            .governor_selection
            .foundation_members
            .contains(&approval.member_wallet)
        {
            return Err(GovernanceError::NotAuthorized);
        }
        ...
        if proposal
            ...
            .iter()
            .any(|a| a.member_wallet == approval.member_wallet)
        {
            return Err(GovernanceError::AlreadyVoted);
        }
...
impl std::fmt::Display for GovernanceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GovernanceError::InsufficientValidators {
                required,
                available,
            } => {
                write!(
            ...
            GovernanceError::InsufficientFoundationMembers {
                required,
                available,
            } => {
                write!(
                    f,
                    "Insufficient foundation members: {} required, {} available",
                    required, available
                )
            }
        }
