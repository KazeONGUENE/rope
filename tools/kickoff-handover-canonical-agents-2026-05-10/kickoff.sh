#!/usr/bin/env bash
#
# Canonical-Agents Handover Rollout — bash fallback (2026-05-10, schema v2)
# =========================================================================
#
...
# ---- API key check (skip in dry-run and direct-deploy) ----
if [[ "$MODE" != "dry-run" ]] && [[ "$MODE" != "direct-deploy" ]] && [[ -z "${CURSOR_API_KEY:-}" ]]; then
  echo "ERROR: CURSOR_API_KEY is not set. Get one from https://cursor.com/dashboard/cloud-agents" >&2
  exit 1
fi
...
echo "Canonical-Agents Handover Rollout (2026-05-10, schema v$SCHEMA_VERSION) — mode=$MODE concurrency=$CONCURRENCY model=$MODEL force=$FORCE force-deploy=$FORCE_DEPLOY skip-preflight=$SKIP_PREFLIGHT" | tee -a "$RUN_LOG_TXT"
